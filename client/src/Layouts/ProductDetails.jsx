...
function ProductDetails() {
  ...
  async function addToUserBag() {
    ...
    toast.success("Item added to bag", TOAST_OPTIONS);
  }
  ...
  function updateRating() {
    const newRating = (
      (details.totalRating + userRating) /
  ...
  useEffect(() => {
    ...
    // get product details
    findProduct(
      `${process.env.REACT_APP_SERVER_URL}/productDetails/${data.id}`
    ).then((product) => {
      // found product
      if (product) {
        setDetails({
          ...
          totalRating: parseFloat(product.totalRating),
        });
      }
    });
  ...
  const imgsArray = data.imgArray.map((img) => {
    return (
      <img
        key={key.current++}
        className={`otherProductImg ${currentImg === img ? "selectedImg" : ""}`}
        src={img}
  ...
  imgsArray.unshift(
    <img
      key={key.current++}
      className={`otherProductImg ${
        currentImg === data.img ? "selectedImg" : ""
      }`}
      src={data.img}
      alt="other product img"
  ...
  const allReviews = details.reviews.map((review) => {
    return (
      <div className="reviewWrapper" key={Math.floor(Math.random() * 100000)}>
        <h5>{review.name}</h5>
        <p>{review.review}</p>
        {user && review.id === user.id && (
          <img
            src={del}
            alt="delete icon"
            className="del-icon"
  ...
  return (
    <div className="productDetailsContainer">
      ...
      <div className="productDetailsImgs">
        ...
        />
        <div className="productDetailsImgArr">{imgsArray}</div>
      </div>
      <div className="productDetailsInfo">
        ...
        <h4>
          Select size{" "}
          {error && (
            <span className="errorMsg">* You need to select a size first</span>
          )}
        </h4>
        <div className="productSizes">
          <button
            className={`sizeBox ${size === "s" ? "selectedBtn" : ""}`}
            onClick={() => setSize("s")}>
            S
          </button>
          <button
            className={`sizeBox ${size === "m" ? "selectedBtn" : ""}`}
            onClick={() => setSize("m")}>
            M
          </button>
          <button
            className={`sizeBox ${size === "l" ? "selectedBtn" : ""}`}
            onClick={() => setSize("l")}>
            L
          </button>
          <button
            className={`sizeBox ${size === "xl" ? "selectedBtn" : ""}`}
            onClick={() => setSize("xl")}>
            XL
          </button>
        </div>
        ...
        <div className="productReviews">
            ...
            {showRating && (
              <div className="flex-col">
                <div className="ratingContainer animate">
                  <img
                    src={userRating >= 1 ? starFill : starHollow}
                    alt="star icon"
                    className="star-icon"
                    onClick={() => setUserRating(1)}
                  />
                  <img
                    src={userRating >= 2 ? starFill : starHollow}
                    alt="star icon"
                    className="star-icon"
                    onClick={() => setUserRating(2)}
                  />
                  <img
                    src={userRating >= 3 ? starFill : starHollow}
                    alt="star icon"
                    className="star-icon"
                    onClick={() => setUserRating(3)}
                  />
                  <img
                    src={userRating >= 4 ? starFill : starHollow}
                    alt="star icon"
                    className="star-icon"
                    onClick={() => setUserRating(4)}
                  />
                  <img
                    src={userRating >= 5 ? starFill : starHollow}
                    alt="star icon"
                    className="star-icon"
          ...
          <div className="reviewsContainer">{allReviews}</div>
        </div>
      </div>
    </div>
  );
