...
function Profile() {
    ...
    function validateChanges() {
        ...
        if(!profileData.email.includes('@') || !profileData.email.includes('.') || profileData.email.length < 10) {
        ...
        if (errorStatus.emailMsg === '' && errorStatus.passwordMsg === '' && errorStatus.firstNameMsg === '' && errorStatus.lastNameMsg === ''){
            ...
            if(profileData.email !== user.email){
                ...
                findUser(`${process.env.REACT_APP_SERVER_URL}/user/${profileData.email}`)
                .then(user => {
                    if(user) {
                ...
                .catch(error => {
                    errorStatus.generalMsg = 'There was a problem saving changes, please try again.'
                    setErrorMessage(errorStatus)
                })
            }
            
            if(errorStatus.emailMsg === '') {
                updateUserData(`${process.env.REACT_APP_SERVER_URL}/updateUser`, {email:profileData.email.toLowerCase(), password: profileData.password, firstName: profileData.firstName, lastName: profileData.lastName, id: user.id})
                .then(result => {
                    // success
                ...
                <div className="user-info">
                    ...
                    <form>
                        <div className="ui-input-container">
                            ...
                            <input id='email' type="email" value={profileData.email} style={errorMessage.emailMsg !== '' ? errorStyle : null} onChange={(e) => {
                                setSave(false)
                        ...
                        <div className="ui-input-container">
                            ...
                            <input id='password' type="password" value={profileData.password} style={errorMessage.passwordMsg !== '' ? errorStyle : null} onChange={(e) => {
                                setSave(false)
                        ...
                        <div className="ui-input-container">
                            ...
                            <input id='firstName' type="text" value={profileData.firstName} style={errorMessage.firstNameMsg !== '' ? errorStyle : null} onChange={(e) => {
                                setSave(false)
                        ...
                        <div className="ui-input-container">
                            ...
                            <input id='lastName' type="text" value={profileData.lastName} style={errorMessage.lastNameMsg !== '' ? errorStyle : null} onChange={(e) => {
                                setSave(false)
                    ...
                    <button disabled = {save} className="ui-saveBtn" onClick={(e) => {
                        e.preventDefault()
                        validateChanges()
                    }}>Save changes</button>
                    <p>{saveStatus}</p>
