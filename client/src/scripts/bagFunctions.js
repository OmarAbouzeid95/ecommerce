...
async function addToBag(id, size, quantity, user, url) {
  ...
  if (!bagItems || bagItems.length === 0) {
    ...
    if (!existing) bagItems.push(product);
  }
...
async function clearBag(url, user) {
  localStorage.setItem("bagItems", JSON.stringify([]));
  const updatedUser = await updateBagItems(url, { ...user, bagItems: [] });
  return updatedUser;
}

export { addToBag, updateBagQuantity, removeFromBag, bagCount, clearBag };
